        ...
        for (size_t y = 0; y < h; ++y) {
            ...
            for (size_t x = 0; x < w; ++x) {
                ...
                if (isInsideEllipsoid(dx, dy, dz, e.ax, e.ay, e.az)) {
                    vol.at(x, y, zi) = std::max(vol.at(x, y, zi), e.hu);
                }
            }
        }
    }
...
Volume Generator3DCPU::generateBrainHU(const Generator3D::Params& params) {
    Volume vol;
    ...
    for (size_t n = 0; n < params.num_ellipsoids; ++n) {
        ...
        const Generator3D::TissueParams& tissue = choose_bone ? params.bone_tissue : params.soft_tissue;
