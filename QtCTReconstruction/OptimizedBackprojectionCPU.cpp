...
void OptimizedBackprojectionCPU::reconstruct(Volume& volume,
    ...
    // Loop Strategy: Project "Reactor V2" (Refined)
    // Minimizes floating point operations and type conversions in the innermost loop.
    ...
    for (int z = 0; z < nz_half; ++z) {
        ...
        for (int y = 0; y < ny; ++y) {
            ...
            float* vol_row_p = &volume.at(0, y, z);
            float* vol_row_n = has_sym ? &volume.at(0, y, z_sym) : nullptr;
            ...
            for (int p_start = 0; p_start < total_projections; p_start += 16) {
                ...
                for (int p = p_start; p < p_end; ++p) {
                    ...
                    const float* row_p = (v0_p >= 0 && v0_p < nh) ? &projs_raw[(p * nh + v0_p) * nw] : nullptr;
                    const float* row_n = (vol_row_n && v0_n >= 0 && v0_n < nh) ? &projs_raw[(p * nh + v0_n) * nw] : nullptr;

                    if (!row_p && !row_n) continue;
                    ...
                    // Positive Slice Voxel Loop with Incremental Coordinate Calculation
                    if (row_p) {
                    ...
                    if (row_n) {
                        ...
                        if (x_start < x_end) {
                            ...
                            for (int x = x_start; x < x_end; ++x) {
                                const int u0 = static_cast<int>(u);
                                const float fu = u - static_cast<float>(u0);
                                vol_row_n[x] += row_n[u0] + fu * (row_n[u0 + 1] - row_n[u0]);
                                u += du;
                            }
                        }
                    }
